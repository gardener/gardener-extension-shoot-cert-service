...
ENSURE_GARDENER_TOOLS_MOD   := $(shell go get github.com/gardener/gardener/hack/tools/tool@$$(go list -m -f "{{.Version}}" github.com/gardener/gardener/hack/tools/tool))
...
.PHONY: generate
generate: $(CONTROLLER_GEN) $(CRD_REF_DOCS) $(HELM) $(MOCKGEN) $(KUSTOMIZE) $(YQ) $(EXTENSION_GEN) $(KUBECTL)
